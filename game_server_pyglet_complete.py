...
from games.ticket_to_ride import TicketToRideGame
...
class PygletGameServer:
    ...
    def _finish_init(self):
        ...
        self.ticket_to_ride_game = TicketToRideGame(self.window.width, self.window.height, self.renderer)
        ...
        try:
            if hasattr(self.ticket_to_ride_game, "set_name_provider"):
                self.ticket_to_ride_game.set_name_provider(self._player_display_name)
        except Exception:
            pass
        ...
        setattr(self.ticket_to_ride_game, "web_ui_only_player_select", True)
        ...
        setattr(self.ticket_to_ride_game, "board_only_mode", True)
    ...
    def _apply_game_selection(self, key: str) -> None:
        ...
        elif key == "ticket_to_ride":
            self.state = "ticket_to_ride"
            self.ticket_to_ride_game.state = "player_select"
            self.ticket_to_ride_game.selection_ui.reset()
        elif key in ("d&d", "dnd"):
    ...
    def _get_active_game(self):
        ...
        if self.state == "ticket_to_ride":
            try:
                if hasattr(self, "ticket_to_ride_game") and hasattr(self.ticket_to_ride_game, "player_names"):
                    self.ticket_to_ride_game.player_names = dict(self.player_names or {})
            except Exception:
                pass
            return self.ticket_to_ride_game
        if self.state == "dnd_creation":
    ...
    def _get_player_select_snapshot(self, game):
        ...
        if self.state == "monopoly":
            ...
            start_enabled = sum(1 for s in slots if s["selected"]) >= 2
        elif self.state == "ticket_to_ride":
            sel_count = sum(1 for s in slots if s["selected"])
            start_enabled = sel_count >= 2 and sel_count <= 5
    ...
    def get_ui_snapshot(self, player_idx: int):
        ...
        if self.state == "ticket_to_ride":
            try:
                tg = self.ticket_to_ride_game
                st = tg.get_public_state(player_idx) if hasattr(tg, "get_public_state") else None
                if isinstance(st, dict):
                    snap["ticket_to_ride"] = st
            except Exception:
                pass
    ...
    def _apply_ui_action(self, client_id: str, data: Dict):
        ...
        if msg_type == "start_game" and getattr(game, "state", None) == "player_select":
            ...
            if self.state == "monopoly" and len(selected_indices) >= 2:
                ...
                game.start_game(selected_indices)
            elif self.state == "ticket_to_ride" and len(selected_indices) >= 2 and len(selected_indices) <= 5:
                game.start_game(selected_indices)
            elif self.state == "dnd_creation":
                dm = self.dnd_dm_seat
    ...
    def on_draw(self):
        ...
        for g in (
            ...
            getattr(self, "ticket_to_ride_game", None),
    ...
    def update(self, dt: float):
        ...
        # Allow Ticket to Ride to consume web "tap" clicks.
        if self.state == "ticket_to_ride" and self.web_cursors:
            try:
                tg = getattr(self, "ticket_to_ride_game", None)
                if tg is not None and hasattr(tg, "handle_pointer_click"):
                    for pidx, cur in list(self.web_cursors.items()):
                        if not bool(cur.get("click")):
                            continue
                        pos = cur.get("pos")
                        if not pos:
                            continue
                        try:
                            tg.handle_pointer_click(int(pidx), int(pos[0]), int(pos[1]))
                        except Exception:
                            pass
                        try:
                            cur["click"] = False
        ...
        if self.state == "menu":
            ...
            try:
                self.ticket_to_ride_game.update(dt)
            except Exception:
                pass
