"""Popup UI drawing for Monopoly game."""
import pygame
from typing import Dict
from config import Colors
from ui_components import RotatedText, draw_circular_progress


class PopupDrawer:
    """Handles drawing all game popups."""
    
    def __init__(self, screen: pygame.Surface):
        self.screen = screen
    
    def draw_buy_prompt(self, popup_data: Dict, panel, buttons: list):
        player = popup_data["player"]
        price = popup_data["price"]
        space = popup_data["space"]
        overlay = pygame.Surface(panel.rect.size, pygame.SRCALPHA)
        overlay.fill((0, 0, 0, 200))
        self.screen.blit(overlay, panel.rect)
        font_title = pygame.font.SysFont("Arial", 22, bold=True)
        font_info = pygame.font.SysFont("Arial", 16)
        font_price = pygame.font.SysFont("Arial", 26, bold=True)
        name = space.data.get("name", "")
        price_text = f"${price}"
        if panel.is_vertical():
            content_rect = panel.get_grid_rect(0.2, 1.5, 2.6, 7.5, 4, 12)
            color = space.data.get("color", (200, 200, 200))
            bar = pygame.Rect(content_rect.x, content_rect.y, content_rect.width, 6)
            pygame.draw.rect(self.screen, color, bar, border_radius=3)
            lines = [name,
                     "Price",
                     price_text,
                     "✓ Can afford" if player.money >= price else "✗ Cannot afford",
                     f"Balance: ${player.money}"]
            # Mixed fonts: render first/title/price separately for emphasis
            # Title line
            RotatedText.draw_stacked_in_rect(self.screen,
                                             [lines[0]],
                                             font_title, Colors.WHITE,
                                             pygame.Rect(content_rect.x,
                                                         content_rect.y + 12,
                                                         content_rect.width,
                                                         50),
                                             panel.orientation,
                                             spacing=10,
                                             center=False)
            # Remaining lines
            RotatedText.draw_stacked_in_rect(self.screen,
                                             lines[1:],
                                             font_info,
                                             Colors.WHITE,
                                             pygame.Rect(content_rect.x,
                                                         content_rect.y + 70,
                                                         content_rect.width,
                                                         content_rect.height - 70),
                                             panel.orientation,
                                             spacing=16,
                                             center=False)
        else:
            content_rect = panel.get_grid_rect(1.2, 0.6, 6.4, 2.4, 12, 4)
            color = space.data.get("color", (200, 200, 200))
            bar = pygame.Rect(content_rect.x, content_rect.y, content_rect.width, 5)
            pygame.draw.rect(self.screen, color, bar, border_radius=2)
            y = content_rect.y + 18
            RotatedText.draw(self.screen, name, font_title, Colors.WHITE,
                             (content_rect.centerx, y), panel.orientation,
                             max_width=content_rect.width - 20)
            y += 42
            RotatedText.draw(self.screen, "Price", font_info, (180,180,180),
                             (content_rect.centerx, y), panel.orientation)
            y += 32
            RotatedText.draw(self.screen, price_text, font_price, Colors.ACCENT,
                             (content_rect.centerx, y), panel.orientation)
            y += 42
            status_text = "✓ Can afford" if player.money >= price else "✗ Cannot afford"
            status_color = (100,220,100) if player.money >= price else (255,100,100)
            RotatedText.draw(self.screen, status_text, font_info, status_color,
                             (content_rect.centerx, y), panel.orientation)
            y += 32
            balance_text = f"Balance: ${player.money}"
            RotatedText.draw(self.screen, balance_text, font_info, (200,200,200),
                             (content_rect.centerx, y), panel.orientation)
        for btn in buttons:
            btn.draw(self.screen)
            for progress_info in btn.get_hover_progress():
                draw_circular_progress(self.screen,
                                       (progress_info["rect"].centerx + 20,
                                        progress_info["rect"].top - 20),
                                       16,
                                       progress_info["progress"],
                                       Colors.ACCENT,
                                       thickness=4)
    
    def draw_card_popup(self, popup_data: Dict, panel, buttons: list):
        card = popup_data["card"]
        deck_type = popup_data["deck_type"]
        overlay = pygame.Surface(panel.rect.size, pygame.SRCALPHA)
        overlay.fill((0,0,0,200))
        self.screen.blit(overlay, panel.rect)
        font_title = pygame.font.SysFont("Arial", 20, bold=True)
        font_text = pygame.font.SysFont("Arial", 15)
        title = "CHANCE" if deck_type == "chance" else "COMMUNITY CHEST"
        body = card.get("text","")
        title_color = (255,200,60) if deck_type == "chance" else (100,180,255)
        if panel.is_vertical():
            content_rect = panel.get_grid_rect(0.25, 1.5, 3, 7.5, 4, 12)
            bar = pygame.Rect(content_rect.x, content_rect.y, content_rect.width, 5)
            pygame.draw.rect(self.screen, title_color, bar, border_radius=2)
            # Wrap body for vertical orientation
            wrap_width = content_rect.height - 80
            words = body.split()
            lines = []
            current = ""
            for w in words:
                test = (current + " " + w).strip()
                if font_text.size(test)[0] <= wrap_width:
                    current = test
                else:
                    if current:
                        lines.append(current)
                    current = w
            if current:
                lines.append(current)
            RotatedText.draw_stacked_in_rect(self.screen,
                                             [title],
                                             font_title,
                                             title_color,
                                             pygame.Rect(content_rect.x,
                                                         content_rect.y + 10,
                                                         content_rect.width,
                                                         50),
                                             panel.orientation,
                                             spacing=10,
                                             center=False)
            RotatedText.draw_stacked_in_rect(self.screen,
                                             lines,
                                             font_text,
                                             Colors.WHITE,
                                             pygame.Rect(content_rect.x,
                                                         content_rect.y + 70,
                                                         content_rect.width,
                                                         content_rect.height - 70),
                                             panel.orientation,
                                             spacing=12,
                                             center=False)
        else:
            content_rect = panel.get_grid_rect(1.2, 0.6, 6.4, 2.4, 12, 4)
            bar = pygame.Rect(content_rect.x, content_rect.y, content_rect.width, 4)
            pygame.draw.rect(self.screen, title_color, bar, border_radius=2)
            title_y = content_rect.y + 18
            RotatedText.draw(self.screen, title, font_title, title_color,
                             (content_rect.centerx, title_y), panel.orientation)
            text_area = content_rect.inflate(-40, -60)
            text_area.y = title_y + 30
            RotatedText.draw_wrapped(self.screen, body, font_text, Colors.WHITE,
                                     text_area, panel.orientation)
        for btn in buttons:
            btn.draw(self.screen)
            for progress_info in btn.get_hover_progress():
                draw_circular_progress(self.screen,
                                       (progress_info["rect"].centerx + 20,
                                        progress_info["rect"].top - 20),
                                       16,
                                       progress_info["progress"],
                                       Colors.ACCENT,
                                       thickness=4)
    
    def draw_properties_popup(self, popup_data: Dict, panel, properties, property_scroll: int, buttons: list):
        player = popup_data["player"]
        overlay = pygame.Surface(panel.rect.size, pygame.SRCALPHA)
        overlay.fill((0,0,0,200))
        self.screen.blit(overlay, panel.rect)
        font_title = pygame.font.SysFont("Arial", 18, bold=True)
        font_label = pygame.font.SysFont("Arial", 13)
        font_small = pygame.font.SysFont("Arial", 12)
        if not player.properties:
            RotatedText.draw(self.screen, "No properties owned", font_label, (180,180,180),
                             (panel.rect.centerx, panel.rect.centery), panel.orientation)
        else:
            prop_idx = player.properties[property_scroll]
            prop = properties[prop_idx]
            if panel.is_vertical():
                content_rect = panel.get_grid_rect(0.25, 1.5, 3, 7.5, 4, 12)
                color = prop.data.get("color", (200,200,200))
                bar = pygame.Rect(content_rect.x, content_rect.y, content_rect.width, 5)
                pygame.draw.rect(self.screen, color, bar, border_radius=3)
                name = prop.data.get("name","")
                details = [
                    f"Type: {prop.data.get('type','property').title()}",
                    f"Value: ${prop.data.get('price',0)}",
                    f"Houses: {prop.houses}" if prop.data.get("type")=="property" else "",
                ]
                if prop.data.get("type")=="property" and prop.houses>0:
                    rent_list = prop.data.get("rent",[0])
                    rent_val = rent_list[min(prop.houses,len(rent_list)-1)]
                    details.append(f"Rent: ${rent_val}")
                details.append(f"Status: {'Mortgaged' if prop.is_mortgaged else 'Active'}")
                details = [d for d in details if d]
                RotatedText.draw_stacked_in_rect(self.screen,
                                                 [name],
                                                 font_title,
                                                 Colors.WHITE,
                                                 pygame.Rect(content_rect.x,
                                                             content_rect.y + 10,
                                                             content_rect.width,
                                                             50),
                                                 panel.orientation,
                                                 spacing=8,
                                                 center=False)
                RotatedText.draw_stacked_in_rect(self.screen,
                                                 details,
                                                 font_label,
                                                 Colors.WHITE,
                                                 pygame.Rect(content_rect.x,
                                                             content_rect.y + 70,
                                                             content_rect.width,
                                                             content_rect.height - 110),
                                                 panel.orientation,
                                                 spacing=12,
                                                 center=False)
                page_rect = pygame.Rect(content_rect.x,
                                        content_rect.bottom - 40,
                                        content_rect.width,
                                        30)
                RotatedText.draw_stacked_in_rect(self.screen,
                                                 [f"{property_scroll + 1} / {len(player.properties)}"],
                                                 font_small,
                                                 (120,120,120),
                                                 page_rect,
                                                 panel.orientation,
                                                 spacing=10,
                                                 center=True)
            else:
                content_rect = panel.get_grid_rect(1.2, 0.6, 6.4, 2.4, 12, 4)
                color = prop.data.get("color",(200,200,200))
                bar = pygame.Rect(content_rect.x, content_rect.y, content_rect.width, 5)
                pygame.draw.rect(self.screen, color, bar, border_radius=2)
                name_y = content_rect.y + 18
                RotatedText.draw(self.screen, prop.data.get("name",""), font_title, Colors.WHITE,
                                 (content_rect.centerx, name_y), panel.orientation,
                                 max_width=content_rect.width - 24)
                y = name_y + 38
                line_spacing = 22
                det_lines = [
                    f"Type: {prop.data.get('type','property').title()}",
                    f"Value: ${prop.data.get('price',0)}"
                ]
                if prop.data.get("type")=="property":
                    det_lines.append(f"Houses: {prop.houses}")
                    if prop.houses>0:
                        rent_list = prop.data.get("rent",[0])
                        rent_val = rent_list[min(prop.houses,len(rent_list)-1)]
                        det_lines.append(f"Rent: ${rent_val}")
                det_lines.append(f"Status: {'Mortgaged' if prop.is_mortgaged else 'Active'}")
                for d in det_lines:
                    RotatedText.draw(self.screen, d, font_label, Colors.WHITE,
                                     (content_rect.centerx, y), panel.orientation)
                    y += line_spacing
                page_text = f"{property_scroll + 1} / {len(player.properties)}"
                RotatedText.draw(self.screen, page_text, font_small, (120,120,120),
                                 (content_rect.centerx, content_rect.bottom - 20), panel.orientation)
        for btn in buttons:
            btn.draw(self.screen)
            for progress_info in btn.get_hover_progress():
                draw_circular_progress(self.screen,
                                       (progress_info["rect"].centerx + 20,
                                        progress_info["rect"].top - 20),
                                       16,
                                       progress_info["progress"],
                                       Colors.ACCENT,
                                       thickness=4)
    
    def draw_build_popup(self, panel, buttons: list):
        """Draw building options popup."""
        overlay = pygame.Surface(panel.rect.size, pygame.SRCALPHA)
        overlay.fill((0, 0, 0, 200))
        self.screen.blit(overlay, panel.rect)
        
        font_title = pygame.font.SysFont("Arial", 22, bold=True)
        title_text = "Building (Coming Soon)"
        RotatedText.draw(self.screen, title_text, font_title, Colors.ACCENT,
                       (panel.rect.centerx, panel.rect.centery), panel.orientation)
        
        for btn in buttons:
            btn.draw(self.screen)
            for progress_info in btn.get_hover_progress():
                center_x = progress_info["rect"].centerx + 20
                center_y = progress_info["rect"].top - 20
                draw_circular_progress(self.screen, (center_x, center_y), 16,
                                     progress_info["progress"], Colors.ACCENT, thickness=4)