...
class GameServer:
        ...
        try:
            async for message in websocket:
                try:
                    ...
                    elif msg_type == "fingertips":
                        self.fingertip_data = data.get("fingertips", [])
                    elif msg_type == "keyboard":
                        await self.process_keyboard(data)
                    elif msg_type == "ping":
    ...
    async def process_keyboard(self, data):
        if self.launcher and self.launcher.current_game:
            if hasattr(self.launcher.current_game, 'handle_text_input'):
                key = data.get("key")
    ...
    async def game_loop(self):
        ...
        while self.running:
            ...
            if self.launcher.state == "menu":
                ...
                self.launcher.handle_game_state(self.fingertip_data, None)
