...
class CardDrawer:
    ...
    @staticmethod
    def draw_card(screen: pygame.Surface, card: Tuple[str, str], x: int, y: int, face_down: bool = False):
        ...
        if face_down:
            ...
            pygame.draw.rect(screen, Colors.WHITE, card_rect, width=2, border_radius=5)
            ...
            pygame.draw.rect(screen, (200, 200, 200), card_rect, width=2, border_radius=5)
            
            font_rank = pygame.font.SysFont(None, 32, bold=True)
            font_suit = pygame.font.SysFont(None, 36)
            
            rank_surf = font_rank.render(rank, True, card_color)
            suit_surf = font_suit.render(suit, True, card_color)
    ...
    def draw_hand(screen: pygame.Surface, hand: List[Tuple[str, str]], x: int, y: int, 
                  spacing: int = 20, dealer_hidden: bool = False):
        for i, card in enumerate(hand):
            face_down = dealer_hidden and i == 1
            CardDrawer.draw_card(screen, card, x + i * spacing, y, face_down)
